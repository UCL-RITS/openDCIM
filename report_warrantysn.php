...
class DeviceWarranty extends Device
{
    function GetWarrantyExpiration() {
        ...
        $selectSQL = "SELECT count(DeviceID) AS NumDevices, 'Unknown warranty' AS NumYears FROM fac_Device WHERE WarrantyExpire=<'1970-01-31' and DeviceType !='Physical Infrastructure' and DeviceType !='Patch Panel';";
        ...
        $selectSQL = "SELECT count(DeviceID) AS NumDevices,'Expired over a year ago' AS NumYears FROM fac_Device WHERE (DATEDIFF(Now(), WarrantyExpire)/365)>=1 AND WarrantyExpire>'1969-12-31' and DeviceType !='Physical Infrastructure' and DeviceType !='Patch Panel';";
        ...
        $selectSQL = "SELECT count(DeviceID) AS NumDevices,'Expired within last year' AS NumYears FROM fac_Device WHERE (DATEDIFF(WarrantyExpire, NOW())/365)<0 AND (DATEDIFF(WarrantyExpire, NOW())/365)>=-1 AND WarrantyExpire>'1969-12-31' and DeviceType !='Physical Infrastructure' and DeviceType !='Patch Panel';";
        ...
        for ($year = 1; $year <= 3; $year++) {
            ...
            $selectSQL = sprintf("SELECT count(DeviceID) AS NumDevices,'%d<=%d remaining years' AS NumYears FROM fac_Device WHERE (DATEDIFF(WarrantyExpire, NOW())/365)>=%d AND (DATEDIFF(WarrantyExpire, NOW())/365)<%d ", $previous_year, $year, $previous_year, $year) and DeviceType !='Physical Infrastructure' and DeviceType !='Patch Panel';
        ...
        $selectSQL = "SELECT count(DeviceID) AS NumDevices,'>3 remaining years' AS NumYears FROM fac_Device WHERE (DATEDIFF(WarrantyExpire, NOW())/365)>3 and DeviceType !='Physical Infrastructure' and DeviceType !='Patch Panel';";
    ...
    function GetDeviceByWarranty($year) {
        ...
        if ($year >= 0) {
            $previous_year = $year - 1;
            $selectSQL = sprintf("SELECT DeviceID, SerialNo, Label, Owner, WarrantyExpire, PrimaryContact FROM fac_Device WHERE (DATEDIFF(WarrantyExpire, NOW())/365)>=%d AND (DATEDIFF(WarrantyExpire, NOW())/365)<%d  and DeviceType !='Physical Infrastructure' and DeviceType !='Patch Panel' ORDER BY Owner, WarrantyExpire, Label;", $previous_year, $year);
            foreach ($dbh->query($selectSQL) as $deviceRow) {
                $deviceList[$deviceRow['DeviceID']] = Device::RowToObject($deviceRow);
            }
        }
        else {
            $selectSQL = "SELECT DeviceID, SerialNo, Label, Owner, WarrantyExpire, PrimaryContact FROM fac_Device WHERE (DATEDIFF(NOW(), WarrantyExpire))>0 AND WarrantyExpire>'1969-12-31'  and DeviceType !='Physical Infrastructure' and DeviceType !='Patch Panel' ORDER BY Owner, WarrantyExpire, Label;";
            foreach ($dbh->query($selectSQL) as $deviceRow) {
                $deviceList[$deviceRow['DeviceID']] = Device::RowToObject($deviceRow);
            }
        }
...
class PDF extends FPDF
{
    ...
    function Header() {
        ...
        $this->Cell(120);
        $this->Cell(30, 20, __("RITS"), 0, 0, 'C');
        $this->Ln(27);
        $this->SetFont($this->pdfconfig->ParameterArray['PDFfont'], '', 10);
        $this->Cell(50, 6, __("Warranty Expiration Report including Serial Numbers"), 0, 1, 'L');
        $this->Cell(50, 6, __("Report generated on") . ': ' . date('d F Y'), 0, 1, 'L');
        $this->Ln(1);
    }
...
$pdf->Cell(0, 18, __("Warranty Status including Serial Numbers"), '', 1, 'C', 0);
$pdf->SetXY(10, 70);
...
for ($year = 1; $year <= 3; $year++) {
    ...
    $headerTags = array(__("Label"),__("Serial"), __("Remaining"), __("Owner"), __("Primary Contact"));
    $cellWidths = array(35, 45, 25, 30, 35);
...
// added by werner
	$pdf->Cell($cellWidths[1], 6, $serial, 'LBRT', 0, 'L', $fill);
            $pdf->Cell($cellWidths[2], 6, $years, 'LBRT', 0, 'L', $fill);
            $pdf->Cell($cellWidths[3], 6, $dept->Name, 'LBRT', 0, 'L', $fill);
            $pdf->Cell($cellWidths[4], 6, $con->FirstName . ' ' . $con->LastName, 'LBRT', 1, 'L', $fill);
...
$pdf->SetFont($config->ParameterArray['PDFfont'], '', 10);
$headerTags = array(__("Label"),__("Serial"), __("Expired"), __("Owner"), __("Primary Contact"));
$cellWidths = array(35, 45, 25, 35);
$maxval = count($headerTags);
for ($col = 0; $col < $maxval; $col++) $pdf->Cell($cellWidths[$col], 7, $headerTags[$col], 1, 0, 'C', 0);
$pdf->Ln();
$fill = 1;
