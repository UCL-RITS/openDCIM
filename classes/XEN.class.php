	...
	*/
	...
	function search($sql){
		...
		foreach($dbh->query($sql) as $row){
			$vmList[$vmCount]=XEN::RowToObject($row);
			$vmCount++;
		}
...
// cceawva this is where we have to add our shell scripts references
//		$namesList=Device::OSS_SNMP_Walk($dev,null,".1.3.6.1.4.1.6876.2.1.1.2");
//	$namesList = Device::shell_exec('xe vm-list | grep name-label | awk '{ print $4 }' | sort');
// cceawva this is where we have to add our shell scripts references
	...
	function UpdateInventory($debug=false){
		...
		$devList=$dev->GetXENDevices();

		foreach($devList as $xenDev){
			if($debug){
				print "Querying host $xenDev->Label @ $xenDev->PrimaryIP...\n";
			}

			if ( $esxDev->SNMPFailureCount < 3 ) {
				$vmList = XEN::RefreshInventory( $xenDev, $debug );
			}
	...
	static function RefreshInventory( $XENDevice, $debug = false ) {
		global $dbh;

		$dev = new Device();
		if ( is_object( $XENDevice ) ) {
			$dev->DeviceID = $XENDevice->DeviceID;
		} else {
			$dev->DeviceID = $XENDevice;
		}
		$dev->GetDevice();
		...
		$vmList = XEN::EnumerateVMs( $dev, $debug );
		if ( count( $vmList ) > 0 ) {
	...
	function GetVMbyIndex() {
		...
		if(!$vmRow=$dbh->query($sql)->fetch()){
			...
			foreach(XEN::RowToObject($vmRow) as $param => $value){
				$this->$param=$value;
			}
			return true;
		}
