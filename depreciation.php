...
function getAge($purchasedate){
    $adjust = (date("md") >= date("md", strtotime($purchasedate))) ? 0 : -1; 
    $years = date("Y") - date("Y", strtotime($purchasedate)); 
    return $years + $adjust; 
}
	...
	if(isset($_REQUEST['projectid'])){
		...
		if($project!=''){
			...
			$sql="SELECT a.ProjectID, a.ProjectName, b.DeviceID, c.Location, b.Position, 
				...
				b.TemplateID, b.Owner, b.WarrantyExpire, f.Value, e.Name as DataCenterName, c.CabinetID, e.DataCenterID 
				FROM fac_Projects a, fac_Device b, fac_Cabinet c, fac_ProjectMembership d, 
				fac_DataCenter e, fac_DeviceCustomValue f  WHERE b.DeviceID=f.DeviceID AND f.AttributeID=(Select AttributeID from fac_DeviceCustomAttribute where Label LIKE '%Purchase Price%') and  a.ProjectID=d.ProjectID AND 
				d.DeviceID=b.DeviceID AND b.Cabinet=c.CabinetID AND 
				c.DataCenterID=e.DataCenterID $projLimit and b.WarrantyExpire !='0000-00-00' and b.DeviceType !='Physical Infrastructure' and b.DeviceType !='Patch Panel' ORDER BY a.ProjectName ASC, 
...
modfied by cceawva, devices need an InstallDate and a Puchase Price attribute to show up here, depreciation at 20% per year estimated, bottoms out at ZERO value.
...
$warrantydate=($row["WarrantyExpire"]);
...
// ((strtotime($dev->AuditStamp)>0)?date('r',strtotime($dev->AuditStamp)):__("Audit not yet completed"))
...
$ageinyears=getAge($row["InstallDate"]);
$lostvalue=($row["Value"]/5) * $ageinyears;
$currentvalue=($row["Value"])-$lostvalue;
...
//$cwarrantydate=date('d-m-Y',strtotime($row["WarrantyExpire"]));
//$cwarrantydate=($row["WarrantyExpire"]);
$cwarranty=($row["WarrantyExpire"]);
$cvalue=($row["Value"]);
